...
class DataTrainingArguments:
    ...
    def __post_init__(self):
        if (
            ...
            raise ValueError(
                "Need at least one of: dataset name, training, validation or test file."
            )

        for file_attr, label in [
            (self.train_file, "train_file"),
            (self.validation_file, "validation_file"),
            (self.test_file, "test_file"),
        ]:
            if file_attr is not None:
                extension = file_attr.split(".")[-1]
                assert extension in ["csv", "json"], (
                    f"`{label}` should be a csv or a json file."
                )
...
def load_and_prepare_dataset(model_args, data_args, training_args):
    # 4. Load dataset
...
def initialize_model_components(model_args):
...
def resolve_column_names(data_args, column_names):
...
def build_transforms(config, feature_extractor):
    transform = Transform(
...
def prepare_split_dataset(
    ...
    if split not in dataset:
        raise ValueError(f"--do_{split} requires a {split} dataset")

    ds = dataset[split]

    max_samples = getattr(
        args, f"max_{'eval' if split != 'train' else 'train'}_samples"
    )
    ...
    def transform_images(examples):
        images = [
            read_image(path, mode=ImageReadMode.RGB) for path in examples[image_column]
        ]
        examples["pixel_values"] = [transform(image) for image in images]
        return examples

    def filter_images(examples):
        return [Image.open(p) or True for p in examples[image_column]]
...
def preprocess_datasets(
    ...
    transform = build_transforms(config, feature_extractor)
...
def train_and_evaluate(trainer, training_args, last_checkpoint, model_args, data_args):
    # 9. Training
    if training_args.do_train:
        checkpoint = None
...
def main():
    ...
    dataset, column_names = load_and_prepare_dataset(
        model_args=model_args, data_args=data_args, training_args=training_args
    )
    model, tokenizer, feature_extractor = initialize_model_components(
        model_args=model_args
    )
    image_column, caption_column = resolve_column_names(
        data_args=data_args, column_names=column_names
    )

    train_dataset, eval_dataset, test_dataset = preprocess_datasets(
        dataset=dataset,
        data_args=data_args,
