...
### Step 2 — `split_videos`
Opens each video with OpenCV (`cv2.VideoCapture`) and reads its FPS, width and height. Writes consecutive segments of `frames_per_segment` frames each with `cv2.VideoWriter`, named `{video_stem}_part_{index:03d}.mp4`. Each segment's absolute `[start_frame, end_frame)` range in the source video is tracked for annotation remapping.

### Step 3 — `upload_segments_and_create_dataset`
Uploads all video segments to the selected datalake with:
- Tag: `video_segment`
...
3. Run — video segments appear in the new dataset version.
