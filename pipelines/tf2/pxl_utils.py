...
def build_tf_example(*variables):
    # Segmentation
    if len(variables) == 12:
        (
            width,
    ...
    if masks is not None:
        feature_dict["image/object/mask"] = _bytes_list_feature(masks)
...
def load_label_map(label_map_path: str) -> dict[int, str]:
    """
    Load a TF Object Detection label map (.pbtxt) and return {id: name}.
    """
    label_map_proto = label_map_util.load_labelmap(label_map_path)
    categories = label_map_util.convert_label_map_to_categories(
        label_map_proto,
        max_num_classes=99999,
        use_display_name=True,
    )
    # categories: [{"id": 1, "name": "cat"}, ...]
    return {int(c["id"]): c["name"] for c in categories}
...
def create_record_files(
    ...
    for split_name, annotations in splits.items():
        ...
        images_dir = images_dir_map.get(split_name)
        if images_dir is None:
            raise ValueError(f"No images_dir provided for split '{split_name}'")

        record_path = os.path.join(record_dir, f"{split_name}.record")
        ...
        with tf.io.TFRecordWriter(record_path) as writer:
            for variables in tfExample_generator(
                ...
                example = build_tf_example(*variables)
...
def log_metrics(experiment, tf_metrics_dir, global_step, metrics_type):
    if metrics_type == "train":
        metrics = tf_events_to_dict(
            dir_path=os.path.join(tf_metrics_dir, "train"), type=metrics_type
        )
        log_type = "Train"
        ...
        metrics = tf_events_to_dict(
            dir_path=os.path.join(tf_metrics_dir, "eval"), type=metrics_type
        )
        log_type = "Validation"
        for metric_name, value in metrics.items():
