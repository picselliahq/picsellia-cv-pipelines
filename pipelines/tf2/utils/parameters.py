...
class TrainingHyperParameters(HyperParameters):
    def __init__(self, log_data: LogDataType):
        ...
        self.learning_rate = self.extract_parameter(
            ["learning_rate"], expected_type=float, default=0.0005
        )
        self.annotation_type = self.extract_parameter(
            ["annotation_type"], expected_type=str, default="polygon"
        )  # ou rectangle
        self.checkpoint_every_n = self.extract_parameter(
            ["checkpoint_every_n"], expected_type=int, default=10
        )
