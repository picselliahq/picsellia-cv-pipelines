...
class LoadStreams:
    ...
    def __init__(
        self, sources="streams.txt", img_size=640, stride=32, auto=True, transforms=None
    ):
        self.mode = "stream"
        self.img_size = img_size
        self.stride = stride
    ...
    def __next__(self):
        ...
        if self.transforms:
            im = np.stack(
                [self.transforms(cv2.cvtColor(x, cv2.COLOR_BGR2RGB)) for x in im0]
            )  # transforms
        else:
            im = np.stack(
                [
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(
        ...
        labels, shapes, self.segments = zip(*cache.values(), strict=False)
        ...
        for i, (label, segment) in enumerate(
            zip(self.labels, self.segments, strict=False)
        ):
            if include_class:
    ...
    def collate_fn(batch):
        im, label, path, shapes = zip(*batch, strict=False)  # transposed
...
def autosplit(
    ...
    for i, img in tqdm(zip(indices, files, strict=False), total=n):
        if (
        ...
        for split in "train", "val", "test":
            ...
            self.stats[split] = {
                ...
                "labels": [
                    ...
                    for k, v in zip(dataset.im_files, dataset.labels, strict=False)
                ],
            }
